...
birds_res <- bivariate_residuals %>%
  filter(filter_condition == "specialists" & group == "birds")|>
...
alpine_biome <- sf::st_read(paste(data_storage_path,"Mountains/Suzette_Alpine_Biome/Alpine_Biome_Suzette.shp", sep = "/"))|>
  ...
  filter(Mountain_range %in% unique(birds_res$Mountain_range))
...
alpine_bubbles_richness <- alpine_biome_df |>
  ...
  left_join(birds_res,by= "Mountain_range")|>
  drop_na()|>
  distinct()

# Merge the data with the shapefile
map_data <- alpine_biome %>%
  left_join(birds_res, by = c("Mountain_range" = "Mountain_range"))
...
pdf("~/Desktop/Datasets/Biodiversity_combined/Visuals/Visuals_Manuscript/Basemaps/Chloropleth/birds_residuals.pdf", width = 12, height = 6)
