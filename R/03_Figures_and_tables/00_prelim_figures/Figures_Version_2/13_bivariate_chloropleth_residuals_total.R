...
generalist_specialist_res <- bivariate_residuals %>%
...
alpine_biome <- sf::st_read(paste(data_storage_path,"Mountains/Suzette_Alpine_Biome/Alpine_Biome_Suzette.shp", sep = "/"))|>
  ...
  filter(Mountain_range %in% unique(generalist_specialist_res$Mountain_range))
...
alpine_bubbles_richness <- alpine_biome_df |>
  ...
  left_join(generalist_specialist_res,by= "Mountain_range")|>
  drop_na()|>
  distinct()

# Merge the data with the shapefile
map_data <- alpine_biome %>%
  left_join(generalist_specialist_res, by = c("Mountain_range" = "Mountain_range"))



# Classify data into 3x3 grid
map_data_bi <- bi_class(map_data, 
                        x = residuals_log_total, 
...
alpine_bubbles_richness <- alpine_bubbles_richness %>%
  bi_class(x = residuals_log_total, y = generalist_total_residuals,  dim = 3,style="quantile")
...
pdf("~/Desktop/Datasets/Biodiversity_combined/Visuals/Visuals_Manuscript/Basemaps/Chloropleth/total_residuals.pdf", width = 12, height = 6)
