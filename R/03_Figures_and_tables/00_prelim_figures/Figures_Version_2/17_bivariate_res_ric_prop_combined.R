...
proportional_richness_results <- sar_combined %>%
  ...
  mutate(
    ...
    proportion_specialists_group = ifelse(generalist_richness_group == 0, 0, (richness_group / generalist_richness_group) * 100),
    
    # Calculate the logged proportions for total richness and group-specific richness
    log_prop_specialists_total = log1p(proportion_specialists_total), # log transform of proportion
...
alp_generalist_specialist <- proportional_richness_results %>%
  ...
  select(Mountain_range, 
         ...
         total_residuals, 
         residuals_log_total,
         proportion_specialists_total, log_prop_specialists_total
  )|>
  distinct()|>
...
alpine_biome <- sf::st_read(paste(data_storage_path, "Mountains/Suzette_Alpine_Biome/Alpine_Biome_Suzette.shp", sep = "/")) %>%
  ...
  filter(Mountain_range %in% unique(alp_generalist_specialist$Mountain_range))
...
alpine_bubbles_richness <- alpine_biome_df %>%
  ...
  left_join(alp_generalist_specialist, by = "Mountain_range") %>%
  drop_na() %>%
  distinct()|>
...
map_data <- alpine_biome %>%
  left_join(alp_generalist_specialist, by = c("Mountain_range" = "Mountain_range"))|>
  mutate(log_prop_specialists_total_jitter = log_prop_specialists_total + rnorm(n(), mean = 0, sd = 1e-5))
...
# 
pdf("~/Desktop/Datasets/Biodiversity_combined/Visuals/Visuals_Manuscript/Basemaps/Chloropleth/vertebrates.pdf", width = 11, height = 6)
