...
mammals_proportion <- proportional_richness_results %>%
  filter(condition == "specialists" & group == "mammals") %>% # Assuming there is a "group" column for species type
...
alpine_biome <- sf::st_read(paste(data_storage_path,"Mountains/Suzette_Alpine_Biome/Alpine_Biome_Suzette.shp", sep = "/"))|>
  ...
  filter(Mountain_range %in% unique(mammals_proportion$Mountain_range))
...
alpine_bubbles_richness <- alpine_biome_df |>
  ...
  left_join(mammals_proportion,by= "Mountain_range")|>
  drop_na()|>
  distinct()

# Merge the data with the shapefile
map_data <- alpine_biome %>%
  left_join(mammals_proportion, by = c("Mountain_range" = "Mountain_range"))
...
pdf("~/Desktop/Datasets/Biodiversity_combined/Visuals/Visuals_Manuscript/Basemaps/Chloropleth/mammals_richness.pdf", width = 12, height = 6)
