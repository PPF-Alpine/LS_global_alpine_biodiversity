...
for (mountain_range in unique(species_richness_modified$Mountain_range)) {
  ...
  bar_plot <- ggplot(test, aes(x = group, y = residuals_log, fill = residuals_log)) +
    ...
    scale_y_continuous(limits = c(-1.3, 1.3)) +  # Set y-axis limits
    scale_fill_gradientn(colors = colors, limits = c(-1.3, 1.3)) +  # Adjusted color scale limits
    labs(title = paste(unique(test$Mountain_range)),
         x = NULL, y = NULL,
         fill = NULL) +
    theme(
...
# 
pdf("~/Desktop/Datasets/output_shared/Vertebrates/bar_plots/specialists.pdf",width=14,height=8)
...
for (mountain_range in unique(species_richness_modified$Mountain_range)) {
  ...
  filename <- paste0("~/Desktop/Datasets/output_shared/Vertebrates/bar_plots/svgs_specialist/residuals_", safe_filename, ".svg")
  
  svg(filename, width = 1, height = 1,bg = "transparent")
