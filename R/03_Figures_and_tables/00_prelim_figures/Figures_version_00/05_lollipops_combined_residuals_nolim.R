...
# test Lollipop plots 
lollipop <- ggplot(test, aes(x = group, y = residuals_log, color = residuals_log)) +
...
for (mountain_range in unique(species_richness_modified$Mountain_range)) {
  ...
  lollipop <- ggplot(test, aes(x = group, y = residuals_log, color = residuals_log)) +
    geom_segment(aes(x = group, xend = group, y = 0, yend = residuals_log), size = 3) +  # Add segments
    geom_point(size = 11) +  # Add points
    geom_hline(yintercept = 0, linetype = "dashed", color = "black", linewidth = 0.25) +
    scale_y_continuous(limits = c(-1.3, 1.1)) +  # Set y-axis limits
    scale_color_gradientn(colors = colors, limits = c(-1.3,1.1)) +  # Adjusted color scale for points and segments
    labs(title = paste(unique(test$Mountain_range)),
         x = NULL, y = NULL,
         color = NULL) +
    theme(
  ...
  plot_list[[mountain_range]] <- lollipop
}


# 
pdf("~/Desktop/Datasets/output_shared/Vertebrates/lollipops/no_lim.pdf",width=14,height=8)
...
for (mountain_range in unique(species_richness_modified$Mountain_range)) {
  ...
  # Create the plot
  lollipop <- ggplot(test, aes(x = group, y = residuals_log, color = residuals_log)) +
    geom_segment(aes(x = group, xend = group, y = 0, yend = residuals_log), size = 1) +  # Add segments
    ...
    theme(
      ...
      axis.title.x = element_blank(),        # Remove x-axis title
      axis.title.y = element_blank(),        # Remove y-axis title
      axis.ticks = element_blank(),  # Thinner axis ticks
  ...
  filename <- paste0("~/Desktop/Datasets/output_shared/Vertebrates/lollipops/svgs_nolim/residuals_", safe_filename, ".svg")
  
  svg(filename, width = 1, height = 1.1,bg = "transparent")
