...
for (df_name in birds_dfs) {
  ...
  df <- df |>
    mutate(min_elevation = as.numeric(replace_na(min_elevation, 0)), # if there is NA as min elevation -- it will be 0 in the database
...
output_path <- paste0(data_storage_path, "/Biodiversity_combined/Expert_validation/Checklists_validated/removed_birds.xlsx")
