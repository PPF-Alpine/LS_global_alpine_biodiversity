...
northern_andes <- mountain_shapes |> 
  filter(Mountain_range == "Northern Andes")


# Download and save temperature chelsa
temp <- Chelsa.Clim.download(save.location ="~/Desktop/Datasets/Mountains/Chelsa",
                             parameter = "bio", 
                             bio.var = c(1))
...
temp_cropped <- raster::crop(temp, northern_andes) # use crop first and then mask --> faster
temp_mountain <- raster::mask(temp_cropped, northern_andes)
...
elevation_mountain <- elevatr::get_elev_raster(northern_andes,
                                              source = "gl3",
                                              clip = "locations", #this clips to polygon 
...
ggplot(combined_data, aes(x = elevation, y = temperature)) +
  geom_point(alpha = 0.5, color = "blue") +  # Adjust alpha for point transparency if there are many points
